...
uint16_t CcmfPlayer::noteToFNum(uint8_t iChannel, uint8_t iNote)
{
...
void CcmfPlayer::cmfNoteOn(uint8_t iChannel, uint8_t iNote, uint8_t iVelocity)
{
	...
	uint16_t iOPLFNum = noteToFNum(iChannel, iNote);
...
void CcmfPlayer::cmfNoteUpdate(uint8_t iChannel)
{
	...
	if ((iChannel > 10) && (this->bPercussive)) {
		...
		if (this->chOPL[iPercChannel].iNoteStart == 0) // Note is not played
			return;

		iBlock = this->chOPL[iPercChannel].iMIDINote / 12;
		if (iBlock > 1) iBlock--; // keep in the same range as the Creative player

		iOPLFNum = noteToFNum(iChannel, this->chOPL[iPercChannel].iMIDINote);
		...
		for (int i = 0; i < iNumChannels; i++) {
			...
			if (this->chOPL[i].iMIDIChannel == iChannel && this->chOPL[i].iNoteStart > 0) {
				iBlock = this->chOPL[i].iMIDINote / 12;
				if (iBlock > 1) iBlock--; // keep in the same range as the Creative player

				iOPLFNum = noteToFNum(iChannel, this->chOPL[i].iMIDINote);
